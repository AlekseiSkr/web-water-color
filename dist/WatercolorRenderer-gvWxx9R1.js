import Yt from "gsap";
const St = (u, t = 0, n = 1) => Math.max(t, Math.min(n, u));
function jt(u, t) {
  const n = u[t], i = u[t + 1], e = u[t + 2], s = Math.max(n, i, e), l = Math.min(n, i, e), r = 128 - 0.168736 * n - 0.331264 * i + 0.5 * e, o = 128 + 0.5 * n - 0.418688 * i - 0.081312 * e;
  return n < 48 || s - l < 12 || o < 128 || o > 181 || r < 72 || r > 137 ? 0 : St((n - i + 18) / 70) * St((o - 128) / 24) * St((137 - r) / 28);
}
function st(u, t, n, i, e, s, l, r) {
...
function Gt(u, t, n) {
  ...
  for (let d = 0; d < s; d++) for (let x = 0; x < e; x++) {
    const S = Math.min(t - 1, Math.round((x + 0.5) * i)), b = Math.min(n - 1, Math.round((d + 0.5) * i));
    l[d * e + x] = jt(u, (b * t + S) * 4) > 0.17 ? 1 : 0;
  }
  let o = [];
...
function Nt(u, t, n, i = "auto") {
  ...
  for (let M = 1; M < n - 1; M++) for (let f = 1; f < t - 1; f++) {
    const d = M * t + f, x = d * 4, S = Math.hypot(s[d + 1] - s[d - 1], s[d + t] - s[d - t]) / 255, b = (Math.max(u[x], u[x + 1], u[x + 2]) - Math.min(u[x], u[x + 1], u[x + 2])) / 255;
    e[d] = St(0.08 + S * 1.45 + b * 0.1, 0, 0.48);
  }
  const l = Gt(u, t, n);
  if (!l) return e;
  const { left: r, right: o, top: a, bottom: m } = l, c = l.width, h = l.height, k = (r + o) * 0.5;
  st(e, t, n, k, a + h * 0.5, c * 0.48, h * 0.5, 0.58);
  const y = It(u, t, n, r + c * 0.1, k - c * 0.03, a + h * 0.25, a + h * 0.53, r + c * 0.32, a + h * 0.4), g = It(u, t, n, k + c * 0.03, o - c * 0.1, a + h * 0.25, a + h * 0.53, r + c * 0.68, a + h * 0.4);
  ...
  const p = It(u, t, n, r + c * 0.25, o - c * 0.25, a + h * 0.62, a + h * 0.86, k, a + h * 0.75);
...
function ut(u, t, n, i, e) {
  let s = 1, l = 1;
  return e === "cover" ? n > i ? s = n / i : l = i / n : n > i ? l = i / n : s = n / i, [(u - 0.5) * s + 0.5, (t - 0.5) * l + 0.5];
}
function vt(u, t, n, i) {
  if (i <= 0) return new Uint8ClampedArray(u);
  const e = new Float32Array(u.length), s = new Uint8ClampedArray(u.length), l = i * 2 + 1;
  for (let r = 0; r < n; r++) for (let o = 0; o < 4; o++) {
    let a = 0;
    for (let m = -i; m <= i; m++) a += u[(r * t + Math.max(0, Math.min(t - 1, m))) * 4 + o];
    for (let m = 0; m < t; m++)
      e[(r * t + m) * 4 + o] = a / l, a -= u[(r * t + Math.max(0, m - i)) * 4 + o], a += u[(r * t + Math.min(t - 1, m + i + 1)) * 4 + o];
  }
...
function At(u, t, n) {
  ...
  for (let r = 0; r < i; r++) {
    const o = r * 4;
    e[r] = u[o] * 8337e-7 + u[o + 1] * 2805e-6 + u[o + 2] * 283e-6;
  }
...
function Kt(u, t, n, i, e, s, l, r) {
  const o = Math.ceil(t / i), a = Math.ceil(n / i), m = o * a, c = new Int16Array(m), h = new Uint8Array(m);
  for (let g = 0; g < a; g++) for (let p = 0; p < o; p++) {
    const M = Math.min(t - 1, Math.round((p + 0.5) * i)), f = Math.min(n - 1, Math.round((g + 0.5) * i)), d = (f * t + M) * 4, x = Math.min(e - 1, Math.floor(u[d] / 256 * e)), S = Math.min(e - 1, Math.floor(u[d + 1] / 256 * e)), b = Math.min(e - 1, Math.floor(u[d + 2] / 256 * e));
    c[g * o + p] = x + S * e + b * e * e;
  }
  const k = [];
  for (let g = 0; g < m; g++) {
    ...
    let d = 0, x = 0, S = 0;
    for (; M.length; ) {
      const b = M.pop(), v = b % o, w = Math.floor(b / o), R = Math.min(t - 1, Math.round((v + 0.5) * i)), D = Math.min(n - 1, Math.round((w + 0.5) * i)), W = (D * t + R) * 4;
      f.push(b), d += u[W], x += u[W + 1], S += u[W + 2];
      for (const E of [b - 1, b + 1, b - o, b + o]) {
        if (E < 0 || E >= m || h[E] || c[E] !== p) continue;
        const $ = E % o;
        Math.abs($ - v) > 1 || (h[E] = 1, M.push(E));
      }
    }
    if (f.length >= l) {
      const b = [d / f.length / 255, x / f.length / 255, S / f.length / 255], v = Math.max(...b), w = Math.min(...b);
  ...
  for (const g of k) {
    ...
    const b = 0.5 * Math.atan2(2 * S, d - x), v = Math.cos(b), w = Math.sin(b), R = -w, D = v, W = /* @__PURE__ */ new Map(), E = Math.max(i, s * 1.22);
    for (const $ of f) {
      const O = $[0] - p, P = $[1] - M, T = O * v + P * w, U = O * R + P * D, X = Math.round(U / E), B = W.get(X);
      B ? (B.min = Math.min(B.min, T), B.max = Math.max(B.max, T), B.v += U, B.count++) : W.set(X, { min: T, max: T, v: U, count: 1 });
    }
    for (const $ of [...W.values()].sort((O, P) => O.v / O.count - P.v / P.count)) {
      let O = $.min + s * 0.65, P = $.max - s * 0.65;
      if ($.count < 2 || P - O < s * 1.65) continue;
      const T = $.v / $.count, U = r.between(-0.1, 0.1) * s, X = [];
      for (let L = 0; L <= 6; L++) {
        const pt = L / 6, at = O + (P - O) * pt, wt = Math.sin(Math.PI * pt) * U, lt = p + v * at + R * (T + wt), ct = M + w * at + D * (T + wt);
        X.push([lt, ct]);
      }
      const B = s * 1.1;
      X.some((L) => L[0] < B || L[0] > t - B || L[1] < B || L[1] > n - B) || y.push({ points: X, color: g.color });
    }
  }
  return y;
}
async function Zt(u, t, n, i = 420, e = "cover", s = "watercolor", l = "auto", r, o = {}) {
  const a = await Vt(u), m = Math.min(1, i / Math.max(a.width, a.height)), c = Math.max(24, Math.round(a.width * m)), h = Math.max(24, Math.round(a.height * m)), k = document.createElement("canvas");
  k.width = c, k.height = h;
  const y = k.getContext("2d", { willReadFrequently: !0 });
  y.drawImage(a, 0, 0, c, h);
  const g = y.getImageData(0, 0, c, h).data, p = { strokeEconomy: 0.72, shapeSimplification: 0.62, strokeLength: 0.58, strokeWidth: 0.58, boundaryFidelity: 0.72, detailBudget: 0.42, detailMultiplier: 1, sourceAccuracy: 0.65, detailPrecision: 0.78, strokeCurvature: 0.34, ...o }, M = 0.48 + p.strokeEconomy * 0.72, f = 1.55 - p.strokeEconomy * 0.77, d = 1.7 - p.strokeEconomy * 0.97, x = 0.65 + p.strokeLength * 0.6, S = 0.65 + p.strokeWidth * 0.6, b = Math.max(1, Math.min(10, p.detailMultiplier)), v = Math.max(0, Math.min(1, p.sourceAccuracy)), w = r ? await r({ data: new Uint8ClampedArray(g), width: c, height: h }) : Nt(g, c, h, l);
  if (w.length !== c * h) throw new Error(`detailMap returned ${w.length} weights; expected ${c * h}.`);
  const R = 1 - v * 0.52, D = vt(g, c, h, Math.round((6 + p.shapeSimplification * 7) * R)), W = vt(g, c, h, Math.round((3 + p.shapeSimplification * 5) * R)), E = vt(g, c, h, Math.round((2 + p.shapeSimplification * 2) * R)), $ = v > 0.86 ? new Uint8ClampedArray(g) : vt(g, c, h, 1), O = [At(E, c, h), At($, c, h), At(g, c, h)], P = new Jt(t), T = a.width / a.height, U = [], X = [], B = Math.min(c, h), L = Math.max(c, h) / 360;
  let pt = 0;
  const at = (I, V, ot, K, ft, Q, yt, Pt) => {
    const mt = Kt(V, c, h, ot * L, K, ft * L, Q, P);
    for (const rt of mt) {
      const A = pt++, Y = ft * L / B * P.between(0.92, 1.08);
      for (let Z = 1; Z < rt.points.length; Z++) {
        const H = rt.points[Z - 1], q = rt.points[Z];
        U.push({
          start: ut(H[0] / c, H[1] / h, T, n, e),
          end: ut(q[0] / c, q[1] / h, T, n, e),
          color: rt.color,
          radius: Y,
  ...
  s === "oil" ? (at(0, D, 9 * M, 3, 23 * S, 3, 0.68, 0), at(1, W, 6 * M, Math.round(5 - p.shapeSimplification), 15 * S, 4, 0.76, 0)) : (at(0, D, 9 * M, 3, 24 * S, 3, 0.027, 0.94), at(1, W, 6 * M, Math.round(5 - p.shapeSimplification), 15 * S, 4, 0.038, 0.78)), (s === "oil" ? [
    { data: E, field: 0, spacing: 21, radius: 8.6, length: 80, opacity: 0.8, water: 0, chance: 0.66, edgeOnly: !1 },
    { data: E, field: 0, spacing: 15, radius: 5.7, length: 58, opacity: 0.85, water: 0, chance: 0.58, edgeOnly: !1 },
    ...
    { data: E, field: 0, spacing: 20, radius: 8, length: 74, opacity: 0.052, water: 0.55, chance: 0.68, edgeOnly: !1 },
    { data: E, field: 0, spacing: 14, radius: 5.1, length: 52, opacity: 0.058, water: 0.43, chance: 0.6, edgeOnly: !1 },
    ...
    const ot = V + 2, K = [], ft = V === 1 ? 1 + (b - 1) * 0.2 : V >= 2 ? b : 1, Q = I.spacing * L * M / Math.sqrt(ft), yt = I.radius * L * S / Math.pow(ft, 0.1), Pt = I.length * L * x, mt = P.between(0, Q), rt = /* @__PURE__ */ new Set();
    if (I.edgeOnly) {
      const A = [];
      for (let H = 0, q = mt; q < h; H++, q += Q) for (let et = 0, j = mt; j < c; et++, j += Q) {
        const G = Math.max(1, Math.min(c - 2, Math.round(j))), _ = Math.max(1, Math.min(h - 2, Math.round(q))), kt = O[I.field], it = Math.hypot(kt.gx[_ * c + G], kt.gy[_ * c + G]), gt = Math.max(0, Math.min(1, w[_ * c + G]));
        it > 0.08 && gt > 0.25 + p.detailPrecision * 0.22 && A.push({ key: H * 1e5 + et, score: Math.pow(gt, 2 + p.detailPrecision * 4) * (0.22 + Math.min(0.8, it)) });
      }
      A.sort((H, q) => q.score - H.score);
      const Y = s === "oil" ? 36 : 44, Z = Math.round(Y * (0.25 + p.detailBudget * 1.8) * d * b);
      A.slice(0, Z).forEach((H) => rt.add(H.key));
    }
    for (let A = 0, Y = mt; Y < h; A++, Y += Q) for (let Z = 0, H = mt; H < c; Z++, H += Q) {
      if (I.edgeOnly && !rt.has(A * 1e5 + Z)) continue;
      const q = H + P.between(-Q * 0.46, Q * 0.46), et = Y + P.between(-Q * 0.46, Q * 0.46), j = Math.max(1, Math.min(c - 2, Math.round(q))), G = Math.max(1, Math.min(h - 2, Math.round(et))), _ = Math.max(0, Math.min(1, w[G * c + j])), kt = V < 2 ? 0.7 + _ * 0.32 : V === 2 ? 0.58 + _ * 0.5 : 0.15 + _ * 1.35;
      if (P.next() > Math.min(1, I.chance * f * kt)) continue;
      const it = O[I.field], gt = Math.hypot(it.gx[G * c + j], it.gy[G * c + j]);
      if (I.edgeOnly && (gt < 0.15 || P.next() > Math.min(1, gt * 2.1)) || s === "oil" && it.luma[G * c + j] > 0.76 && gt < 0.07 && ot < 5) continue;
      let N = -it.gy[G * c + j], J = it.gx[G * c + j];
      const Ct = Math.hypot(N, J);
      if (Ct < 0.015) {
        const C = s === "oil" ? 0.1 + Math.sin(q * 0.018 + t) * 0.2 + Math.cos(et * 0.016 - t * 0.7) * 0.16 : P.next() * Math.PI;
      ...
      const Ft = V >= 3 ? 1.16 - _ * 0.38 : 1, Rt = Pt * Ft * P.between(s === "oil" ? 0.84 : 0.72, s === "oil" ? 1.18 : 1.28), $t = ot >= 5 ? 2 : s === "oil" ? 6 : 5, ht = [], Mt = qt(I.data, c, h, q, et), zt = [0.3, 0.26, 0.2, 0.14, 0.1][V] * (1.3 - p.boundaryFidelity * 0.55) * (1.22 - v * 0.42), Tt = (C) => {
        let tt = 0, z = 0;
        for (let nt = 3; nt <= Rt * 0.5; nt += 3) {
          const dt = q + N * nt * C, Wt = et + J * nt * C;
          if (dt < 1 || dt >= c - 1 || Wt < 1 || Wt >= h - 1) break;
          const Et = qt(I.data, c, h, dt, Wt);
          if (Math.hypot(Et[0] - Mt[0], Et[1] - Mt[1], Et[2] - Mt[2]) > zt) {
            if (++z >= 2) break;
          } else
            tt = nt, z = 0;
        }
      ...
      if (xt + Ot < Math.max(I.radius * 2.4, Rt * 0.28)) continue;
      const Bt = (s === "oil" ? 0.055 : 0.12) * (0.35 + p.strokeCurvature * 1.9), Ut = P.between(-Bt, Bt);
      for (let C = 0; C <= $t; C++) {
        const tt = -xt + (xt + Ot) * C / $t, z = Math.sin(C / $t * Math.PI) * Ut * (xt + Ot);
        ht.push([q + N * tt - J * z, et + J * tt + N * z]);
      }
      const bt = yt * Ft * (s === "oil" ? 3.2 : 1.45);
      if (ht.some((C) => C[0] < bt || C[0] > c - bt || C[1] < bt || C[1] > h - bt)) continue;
      const Qt = I.edgeOnly ? Mt.map((C) => C * 0.8) : Mt, Lt = [], Xt = pt++;
      for (let C = 1; C < ht.length; C++) {
        const tt = ut(ht[C - 1][0] / c, ht[C - 1][1] / h, T, n, e), z = ut(ht[C][0] / c, ht[C][1] / h, T, n, e);
        Lt.push({ start: tt, end: z, color: Qt, radius: yt * Ft / B * P.between(0.88, 1.12), opacity: I.opacity * (0.88 + _ * 0.18) * P.between(0.9, 1.1), water: I.water * P.between(0.85, 1.15), layer: ot, strokeId: Xt });
      }
      K.push(Lt);
    }
    for (let A = K.length - 1; A > 0; A--) {
      const Y = Math.floor(P.next() * (A + 1));
      [K[A], K[Y]] = [K[Y], K[A]];
    }
    K.forEach((A) => U.push(...A)), X.push(U.length);
  });
...
const _t = {
  ...
  gloss: 0.48
}, F = (u, t = 0, n = 1) => Math.min(n, Math.max(t, u)), Dt = () => Math.random() * 1e4, te = (u, [t, n, i, e]) => {
  const s = (a, m, c) => 3 * (1 - a) * (1 - a) * a * m + 3 * (1 - a) * a * a * c + a * a * a, l = (a, m, c) => 3 * (1 - a) * (1 - a) * m + 6 * (1 - a) * a * (c - m) + 3 * a * a * (1 - c), r = F(u);
  let o = r;
  for (let a = 0; a < 5; a++) {
    const m = l(o, t, i);
...
class se {
  ...
  async setImage(t) {
    ...
    const n = 1 + (Math.sqrt(F(this.options.detailMultiplier, 1, 10)) - 1) * (0.35 + F(this.options.sourceAccuracy) * 0.65), i = Math.min(720, Math.round(this.options.analysisResolution * n));
    this.plan = await Zt(t, this.seed, this.canvasAspect(), i, this.options.imageFit, this.options.mode, this.options.detailFocus, this.options.detailMap, {
    ...
    }), !this.destroyed && (this.buildTimelineWork(), this.canvas.dataset.watercolorSegments = String(this.plan.segments.length), this.canvas.dataset.watercolorLayerEnds = this.plan.layerEnds.join(","), this.clearCheckpoints(), this.resetPainting(), this.setPhase("painting"));
  }
  play() {
  ...
  paintOilStroke(t, n, i = this.pigmentContext) {
    ...
    const h = Math.max(0.7, s.radius * this.height), k = s.color.map((v) => Math.round(F(v) * 255)), y = this.mixProfile(o, k), g = this.averageColors(y), p = F(s.opacity) * (0.55 + this.options.paintLoad * 0.65) * (m === "dry" ? 0.62 : 1), M = 0.45 + this.options.granulation * 0.85, f = g.map((v) => Math.round(v * 0.55)), d = g.map((v) => Math.round(v + (255 - v) * 0.42));
    if (i.save(), i.lineCap = "round", i.lineJoin = "round", this.plan) {
      const [v, w, R, D] = this.plan.bounds;
      i.beginPath(), i.rect(v * this.width, w * this.height, (R - v) * this.width, (D - w) * this.height), i.clip();
    }
    ...
    for (let v = 0; v < b; v++) {
      ...
      const R = y[Math.min(y.length - 1, Math.floor(e.next() * y.length))], D = (v / (b - 1) - 0.5) * 0.16 + (e.next() - 0.5) * 0.06, W = R.map((E) => Math.round(F(E / 255 + D) * 255));
  ...
  paintOilTrails(t, n, i, e, s, l) {
    ...
    for (let M = 0; M < p; M++) {
      ...
      t.strokeStyle = `rgba(${e[0]},${e[1]},${e[2]},${l * (0.2 + s.next() * 0.22)})`, t.lineWidth = Math.max(0.35, i * (0.045 + s.next() * 0.08)), this.strokePath(t, [[o[0] + y * d, o[1] + g * d], [r[0] + y * d, r[1] + g * d], [r[0] + y * d + h * x, r[1] + g * d + k * x]]), t.stroke();
    }
  }
  offsetPath(t, n, i) {
    return t.map((e, s) => {
  ...
  fillOilBody(t, n, i, e, s) {
    ...
    const l = [], r = [];
    n.forEach((o, a) => {
      const m = n[Math.max(0, a - 1)], c = n[Math.min(n.length - 1, a + 1)], h = c[0] - m[0], k = c[1] - m[1], y = Math.max(1, Math.hypot(h, k)), g = a / (n.length - 1), p = Math.pow(Math.max(0, Math.sin(Math.PI * g)), e === "tap" ? 0.25 : 0.48), M = e === "dry" ? 0.12 : e === "tap" ? 0.3 : 0.24, f = (M + (1 - M) * p) * (1 + 0.065 * Math.sin(g * Math.PI * 5 + s) + 0.035 * Math.sin(g * Math.PI * 11 - s * 0.7)), d = -k / y * i * f, x = h / y * i * f;
      l.push([o[0] + d, o[1] + x]), r.push([o[0] - d, o[1] - x]);
  ...
  paintOilSurface(t, n, i, e, s, l, r) {
    ...
    for (let m = 0; m < Math.ceil(a * (0.1 + this.options.gloss * 0.55)); m++) {
      const c = Math.min(n.length - 2, Math.floor(s.next() * (n.length - 1))), h = n[c], k = n[c + 1], y = k[0] - h[0], g = k[1] - h[1], p = Math.max(1, Math.hypot(y, g)), M = -g / p, f = y / p, d = e[Math.floor(s.next() * e.length)], x = -i * (0.15 + s.next() * 0.45);
      t.strokeStyle = `rgba(${Math.round(d[0] + (255 - d[0]) * 0.72)},${Math.round(d[1] + (255 - d[1]) * 0.72)},${Math.round(d[2] + (255 - d[2]) * 0.72)},${0.085 * l * r * this.options.gloss})`, t.lineWidth = Math.max(0.35, i * 0.035), t.beginPath(), t.moveTo(h[0] + M * x, h[1] + f * x), t.lineTo(h[0] + M * x + y * 0.65, h[1] + f * x + g * 0.65), t.stroke();
    }
  }
  paintWatercolorStroke(t, n, i = 0, e = this.pigmentContext) {
    ...
    if (e.save(), this.plan) {
      const [f, d, x, S] = this.plan.bounds;
      e.beginPath(), e.rect(f * this.width, d * this.height, (x - f) * this.width, (S - d) * this.height), e.clip();
    }
    ...
    for (let f = 0; f < p; f++) {
      ...
      const d = (f / Math.max(1, p - 1) - 0.5) * m * 1.5 + (l.next() - 0.5) * m * 0.12;
      e.strokeStyle = `rgba(${c[0]},${c[1]},${c[2]},${h * (0.2 + l.next() * 0.6) * g})`, e.lineWidth = Math.max(0.28, m * (0.025 + l.next() * 0.07)), this.strokePath(e, this.offsetPath(a, d, 0)), e.stroke();
    }
    const M = Math.max(1, Math.min(9, Math.round(a.length * 0.24 * r)));
    for (let f = 0; f < M; f++) {
      const d = Math.min(a.length - 2, Math.floor(l.next() * (a.length - 1))), x = a[d], S = a[d + 1], b = S[0] - x[0], v = S[1] - x[1], w = Math.max(1, Math.hypot(b, v)), R = -v / w, D = b / w, W = (l.next() - 0.5) * m * 1.35;
      this.paperHeight(x[0], x[1]) < 0.5 && (e.fillStyle = `rgba(${Math.round(c[0] * 0.64)},${Math.round(c[1] * 0.64)},${Math.round(c[2] * 0.64)},${h * (0.18 + l.next() * 0.25)})`, e.beginPath(), e.arc(x[0] + R * W, x[1] + D * W, Math.max(0.3, m * (0.025 + l.next() * 0.07)), 0, Math.PI * 2), e.fill());
    }
    ...
    for (let f = 0; f < Math.min(6, M); f++) {
      const d = a[Math.floor(l.next() * a.length)];
      this.paperHeight(d[0], d[1]) > 0.56 && (e.beginPath(), e.arc(d[0] + (l.next() - 0.5) * m, d[1] + (l.next() - 0.5) * m, Math.max(0.25, m * (0.018 + l.next() * 0.045)), 0, Math.PI * 2), e.fill());
    }
    e.restore();
  }
  flowPoint(t, n, i) {
